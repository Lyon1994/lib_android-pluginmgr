...
public class PluginManager implements FileFilter {
	...
	private PluginManager(Context context) {
		if (!isMainThread()) {
			throw new IllegalThreadStateException(
					"PluginManager must init in UI Thread!");
		}
		this.context = context;
		File optimizedDexPath = context.getDir(
		...
		dexInternalStoragePath = context.getDir(
				Globals.PRIVATE_PLUGIN_ODEX_OUTPUT_DIR_NAME,
				Context.MODE_PRIVATE);
		uiHandler = new Handler(Looper.getMainLooper());
		DelegateActivityThread delegateActivityThread = DelegateActivityThread
				.getSingleton();
		...
		if (originInstrumentation instanceof PluginInstrumentation) {
			...
			pluginInstrumentation = new PluginInstrumentation(
	...
	private static void checkInit() {
		if (SINGLETON == null) {
			throw new IllegalStateException(
					"Please init the PluginManager first!");
		}
	}
	...
	public static void init(Context context) {
		if (SINGLETON != null) {
			Log.w(TAG,
					"PluginManager have been initialized, YOU needn't initialize it again!");
			return;
		}
	 ...
	 * @return 插件信息
	 */
	public PlugInfo tryGetPluginInfo(String plugPkg)
	...
	private PlugInfo buildPlugInfo(File pluginApk, String pluginId,
		...
		PluginClassLoader pluginClassLoader = new PluginClassLoader(info,
				dexPath, dexOutputPath, getPluginLibPath(info)
	...
	private void attachBaseContext(PlugInfo info, Application app) {
		try {
			...
			mBase.set(app, new PluginContext(context.getApplicationContext(),
					info));
		} catch (Throwable e) {
			e.printStackTrace();
		}
	 ...
	 * @return
	 */
	private Application makeApplication(PluginClassLoader pluginClassLoader,
			String appClassName) {
		if (appClassName != null) {
			try {
				return (Application) pluginClassLoader.loadClass(appClassName)
						.newInstance();
	...
	private void copyApkToPrivatePath(File pluginApk, File targetPutApk) {
		if (targetPutApk.exists() && pluginOverdueVerifier != null
				&& pluginOverdueVerifier.isOverdue(pluginApk, targetPutApk)) {
			return;
		}
	 ...
	 * 设置插件Activity生命周期监听器
	 * 
	 * @param pluginActivityLifeCycleCallback
	 *            插件Activity生命周期监听器
	 */
	 ...
	 *            插件验证校检器
	 */
	public void setPluginOverdueVerifier(
	...
	public void startMainActivity(Context from, PlugInfo plugInfo) {
		...
		if (activityInfo == null) {
			throw new ActivityNotFoundException(
					"Cannot find Main Activity from plugin.");
		}
		...
		CreateActivityData createActivityData = new CreateActivityData(
				mainActivityName, plugInfo.getPackageName());
		Intent intent = new Intent(from,
				Globals.selectDynamicActivity(activityInfo));
		intent.putExtra(Globals.FLAG_ACTIVITY_FROM_PLUGIN, createActivityData);
		from.startActivity(intent);
	}

	public void startMainActivity(Context from, String pluginPkgName)
			throws PluginNotFoundException, ActivityNotFoundException {
		PlugInfo plugInfo = tryGetPluginInfo(pluginPkgName);
		startMainActivity(from, plugInfo);
	}

	public void startActivity(Context from, PlugInfo plugInfo,
			String targetActivity) {
		ActivityInfo activityInfo = plugInfo
				.findActivityByClassName(targetActivity);
		if (activityInfo == null) {
			throw new ActivityNotFoundException(
					"Cannot find "
							+ targetActivity
							+ " from plugin, could you declare this Activity in plugin?");
		}
		CreateActivityData createActivityData = new CreateActivityData(
				targetActivity, plugInfo.getPackageName());
		Intent intent = new Intent(from,
				Globals.selectDynamicActivity(activityInfo));
		intent.putExtra(Globals.FLAG_ACTIVITY_FROM_PLUGIN, createActivityData);
		from.startActivity(intent);
	...
	public void startActivity(Context from, String pluginPkgName,
			String targetActivity) throws PluginNotFoundException,
	...
	public void dump() {
		Log.d(TAG,
				pluginPkgToInfoMap.size()
						+ " Plugins is loaded, "
						+ Arrays.toString(pluginPkgToInfoMap.values().toArray()));
	}
